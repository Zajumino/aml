...
def create_parser():
    ...
    parser.add_argument('--fc', nargs='+', type=int, default=[100, 5], help='Number of fully connected units per layer (sequence of ints)')
...
def generate_fname(args, params_str):
    ...
    hidden_str = '_'.join(str(x) for x in args.fc)
    
    # Conv configuration
    conv_size_str = '_'.join(str(x) for x in args.kernel_size)
    conv_filter_str = '_'.join(str(x) for x in args.filters)
    pool_str = '_'.join(str(x) for x in args.strides)
    ...
    return "%s/image_%s%sCsize_%s_Cfilters_%s_Pool_%s_Pad_%s_hidden_%s_%s%s%s%sntrain_%02d_rot_%02d"%(args.results_path,
                                                                                           experiment_type_str,
                                                                                           label_str,
                                                                                           conv_size_str,
                                                                                           conv_filter_str,
...
def execute_exp(args=None):
    ...
    model = create_cnn_classifier_network(input_shape=args.image_size, in_channels=nchannels, out_classes=n_classes,
                                          ...
                                          lrate=args.lrate, loss='sparse_categorical_crossentropy', metrics=[tf.keras.metrics.SparseCategoricalAccuracy()])
